...
class InferArgsParser(argparse.ArgumentParser):
    """推理专用的参数解析器"""
    
    def __init__(self):
        super(InferArgsParser, self).__init__(formatter_class=argparse.RawDescriptionHelpFormatter)
        self.add_argument("-c", "--config", help="configuration file to use")
        self.add_argument("-o", "--opt", nargs="+", help="set configuration options")

    def parse_args(self, argv=None):
        args = super(InferArgsParser, self).parse_args(argv)
...
def preprocess_infer():
    ...
    支持的命令行参数:
        -c/--config: 指定配置文件路径
        -o/--opt: 覆盖配置参数，格式为 key=value
    
    示例:
        python predict_system.py -c args.yml -o Global.image_dir=./images/
    
    Returns:
        config: 解析后的配置字典
        logger: 日志记录器
    """
    parser = InferArgsParser()
    ...
    if args.config is None:
        ...
        default_config = os.path.join(current_dir, "args.yml")
        if os.path.exists(default_config):
            args.config = default_config
        else:
            raise ValueError(
                "Please specify --config=configure_file_path or place args.yml in tools/infer/\n"
                "Example: python predict_system.py -c tools/infer/args.yml"
            )
    
    config = load_config(args.config)
    config = merge_config(config, args.opt)
    
    # 创建logger
    global_config = config.get("Global", {})
    log_file = None
    show_log = global_config.get("show_log", True)
